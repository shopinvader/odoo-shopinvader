...
class ShopinvaderProduct(models.Model):
    ...
    cross_selling_ids = fields.Many2many(
        comodel_name='shopinvader.variant', multi='shopinvader_link',
    ...
    def _compute_shopinvader_link(self):
        for record in self:
            record.cross_selling_ids =\
                record._get_shopinvader_variant_link('cross_sell')
            record.up_selling_ids =\
                record._get_shopinvader_variant_link('up_sell')
