...
class ProductImageImportWizard(models.TransientModel):
    ...
    def import_file(self):
        ...
        if headers != HEADERS.split(DELIMITER):
            raise Warning('Invalid CSV file headers found! Expected: %s' % HEADERS)
	...
	relation_obj = self.env['product.image.relation']
        ...
        for row in reader:
            ...
            try:
                ...
                errors.append('%s: impossible to retrieve file "%s"' % (default_code, image_path))
                continue
