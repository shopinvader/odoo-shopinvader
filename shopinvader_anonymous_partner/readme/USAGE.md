...
> `odoo.http.request.future_response` and FastAPI `Response` objects.

`_get_anonymous_partner__cookie(cookies)`

> Return the partner record corresponding to the
> `shopinvader-anonymous-partner` cookie in the current request, if any.
> Returns an empty record set if the cookie is not set or if the
