...
class TestShopinvaderSaleLoyaltyCommon(CommonCase):
    ...
    def _create_program_choice_reward(self, product):
        return self.env["loyalty.program"].create(
            {
                "name": "Buy 1 product, choose 10% on all or 25% on cheapest",
    ...
    def _create_program_free_product_choice(self, rule_product, product_tag):
        return self.env["loyalty.program"].create(
            {
