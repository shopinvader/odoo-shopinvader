...
class ShopinvaderSaleProfile(models.Model):
    ...
    code = fields.Char(
        required=True,
        help="Unique code of the sale profile",
        index=True,
    )
    ...
    _sql_constraints = [
        ('constraint_unique_code', 'unique(backend_id, code)',
    ...
    def _check_default(self):
        ...
        count_data = dict((item[group_by][0], item.get('__count', 0))
    ...
    def _check_unique_pricelist_fposition(self):
        ...
        for record in self:
            profiles = all_profiles.filtered(
                lambda r, rec=record: r.backend_id.id == rec.backend_id.id and
                r.pricelist_id.id == rec.pricelist_id.id and
                any([fpos.id in rec.fiscal_position_ids.ids for fpos in
                     r.fiscal_position_ids]))
            if len(profiles) > 1:
                message = _("Pricelist and fiscal position combination must "
