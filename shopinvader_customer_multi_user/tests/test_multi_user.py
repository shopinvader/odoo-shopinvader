...
class TestCustomer(TestCustomerCommon):
    ...
    def test_controller_find_partner(self):
        self.backend.customer_multi_user = True
        data = dict(
        ...
        with MockRequest(self.env):
            found = ctrl._find_partner(self.backend, data["email"])
        self.assertEqual(found, self.company_binding)
        self.assertEqual(found.record_id, partner1.parent_id)
