...
from odoo.addons.shopinvader_v1_base.models.track_external_mixin import (
    TrackExternalMixin,
)
...
class UtilsMixin(object):
    ...
    def _create_invader_partner(env, **kw):
        values = {
            "backend_id": env.ref("shopinvader_v1_base.backend_1").id,
        }
        values.update(kw)
...
class ShopinvaderRestCase(BaseRestCase):
    def setUp(self, *args, **kwargs):
        ...
        self.backend = self.env.ref("shopinvader_v1_base.backend_1")
        # To ensure multi-backend works correctly, we just have to create
        # a new one on the same company.
        self.backend_copy = self.env.ref("shopinvader_v1_base.backend_2")
...
class CommonTestDownload(object):
    ...
    def _test_download_allowed(self, service, target):
        ...
        """
        with mock.patch(
            "odoo.addons.shopinvader_v1_base.services."
