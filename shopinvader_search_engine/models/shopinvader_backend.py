...
class ShopinvaderBackend(models.Model):
    ...
    @api.depends("s_backend_id")
    def _compute_hide_se_settings(self):
        for rec in self:
            rec.hide_se_settings = bool(rec.s_backend_id)

    def _inversese_backend_id(self):
        for rec in self:
            if rec.s_backend_id:
                rec.update({
                    "se_backend_id": rec.s_backend_id.se_backend_id,
                    "lang_ids": rec.s_backend_id.lang_ids,
                })
