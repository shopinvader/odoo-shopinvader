...
class TestBaseUrl(SingleTransactionCase):
    ...
    def test_change_name(self):
        ...
        url_key = self.env['url.url'].search([('model_id','=', "product.template,9"),('redirect','=',False)]).url_key
    ...
    def test_get_object(self):
        ...
        product._set_url()
        url1 = self.env['url.url'].search([('model_id','=', "product.template,9"),('redirect','=',False)]).url_key
        
        product.url_key = u"De Jolie éperviere" #(de-jolie-eperviere)  
        product._set_url()
        url2 = self.env['url.url'].search([('model_id','=', "product.template,9"),('redirect','=',False)]).url_key
